...
const AnimatedBackground: React.FC<AnimatedBackgroundProps> = ({ activeSection = 'home' }) => {
  ...
  return (
    <div className="fixed inset-0 overflow-hidden pointer-events-none" style={{ zIndex: -1 }}>
      ...
      {/* Dynamic mouse interaction glow - MUCH MORE VISIBLE with smooth transitions */}
      <motion.div 
      ...
      {/* Striking geometric patterns - MUCH MORE VISIBLE with smooth transitions */}
      <div className="absolute inset-0">
        {[...Array(3)].map((_, i) => (
          <motion.div
            key={`geo-${i}-${activeSection}`} // Force re-render on section change
      ...
      {/* Enhanced floating orbs - SUPER VISIBLE with smooth transitions */}
      {[...Array(6)].map((_, i) => (
        <motion.div
          key={`orb-${i}-${activeSection}`} // Force re-render on section change
      ...
      {/* Animated grid pattern - MORE SUBTLE DOTS */}
      <motion.div
        className="absolute inset-0"
        style={{
      ...
      {/* Dynamic corner gradients - SUPER VISIBLE with smooth transitions */}
      <motion.div
        key={`corner-tl-${activeSection}`} // Force re-render on section change
      ...
      <motion.div
        key={`corner-br-${activeSection}`} // Force re-render on section change
      ...
      {/* Pulsing center accent - SUPER VISIBLE with smooth transitions */}
      <motion.div
        key={`center-${activeSection}`} // Force re-render on section change
