...
const AnimatedBackground: React.FC<AnimatedBackgroundProps> = ({ activeSection = 'home' }) => {
  ...
  const getInterpolatedTheme = useCallback(() => {
    const currentIndex = themeKeys.indexOf(activeSection as keyof typeof sectionThemes);
    const nextIndex = (currentIndex + 1) % themeKeys.length;
    
    const currentTheme = sectionThemes[themeKeys[currentIndex]];
    const nextTheme = sectionThemes[themeKeys[nextIndex]];
    
    // Use a smoother interpolation factor based on scroll within section
    const factor = Math.min(scrollProgress * 0.3, 1); // Adjust 0.3 to control transition speed
  ...
  }, [activeSection, scrollProgress, themeKeys]);
