...
const AnimatedBackground: React.FC<AnimatedBackgroundProps> = ({ activeSection = 'home' }) => {
  ...
  // Lightweight color schemes
  const sectionThemes = {
    home: { primary: '#00d4ff', glow: '#0066ff20' },
    about: { primary: '#ff0080', glow: '#ff008020' },
    experience: { primary: '#7000ff', glow: '#7000ff20' },
    projects: { primary: '#00ff88', glow: '#00ff8820' },
    contact: { primary: '#ffaa00', glow: '#ffaa0020' }
  };
  ...
  return (
    <div className="fixed inset-0 overflow-hidden pointer-events-none" style={{ zIndex: -1 }}>
      ...
      <motion.div 
        ...
        style={{
          background: `radial-gradient(circle at ${mousePosition.x}px ${mousePosition.y}px, ${currentTheme.glow} 0%, transparent 50%)`
        }}
        animate={{
          opacity: [0.2, 0.4, 0.2],
        }}
      ...
      <motion.div
        ...
        style={{
          background: `linear-gradient(45deg, transparent 60%, ${currentTheme.glow} 100%)`
        }}
      />
    </div>
  );
  ...
  useEffect(() => {
    setParticles(prevParticles =>
      prevParticles.map(particle => {
        ...
        return {
          ...particle,
          color: randomColor || currentTheme.primary
        };
      })
    );
