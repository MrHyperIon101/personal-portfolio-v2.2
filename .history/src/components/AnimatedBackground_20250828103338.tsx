...
const AnimatedBackground: React.FC<AnimatedBackgroundProps> = ({ activeSection = 'home' }) => {
  ...
  return (
    <div className="fixed inset-0 overflow-hidden pointer-events-none" style={{ zIndex: -1 }}>
      ...
      {/* Mouse glow effect - slightly darker but still visible */}
      <motion.div 
        className="absolute inset-0"
        style={{
          background: `radial-gradient(circle at ${mousePosition.x}px ${mousePosition.y}px, ${theme.glow} 0%, ${theme.secondary}30 30%, transparent 70%)`
        }}
        animate={{
          opacity: [0.5, 0.8, 0.5],
        }}
      ...
      {/* Grid pattern - visible but darker */}
      <motion.div
        className="absolute inset-0"
        style={{
          backgroundImage: `radial-gradient(circle at 15px 15px, ${theme.primary} 1.5px, transparent 1.5px)`,
