...
export default function Container(props: ContainerProps) {
  ...
  return (
    <>
      ...
      {/* Desktop Floating Pill Navbar - Only on main page */}
      {isMainPage && !isLoading && (
        <motion.nav
          data-navbar="true"
          className={cn(
            "z-[9999] hidden sm:flex relative overflow-hidden group",
      ...
      {/* Mobile Navbar - Just Logo - Only on main page */}
      {isMainPage && !isLoading && (
        <motion.nav
          data-navbar="true"
          className="z-[9999] sm:hidden px-2 py-3 bg-black/80 backdrop-blur-md border-b border-white/10"
          initial={{ y: -100 }}
          animate={{ y: 0 }}
          transition={{ 
            type: "spring",
        ...
        >
          <div className="flex items-center justify-start px-4">
            <Link
              ...
              onClick={handleClick}
              className={cn(
                "liquid-glass inline-block px-4 py-2 rounded-full border border-white/30 relative overflow-hidden group",
                "bg-transparent border-2",
                "transition-all duration-300 ease-out"
              )}
            >
              {/* Mobile rotating border gradient */}
              <div className="absolute inset-0 rounded-full overflow-hidden">
                <div 
                  className="absolute inset-[-2px] rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-500"
                  style={{
              ...
              {/* Mobile inner content background */}
              ...
              <GlitchText className="text-white font-bold text-lg relative z-10">
                Souvik Bagchi
              </GlitchText>
            </Link>
          </div>
