...
const AnimatedBackground: React.FC<AnimatedBackgroundProps> = ({ activeSection = 'home' }) => {
  ...
  const themeKeys = Object.keys(sectionThemes) as (keyof typeof sectionThemes)[];
  
  // Calculate interpolated colors based on scroll
  const getInterpolatedTheme = useCallback(() => {
    const validSectionKeys = ['home', 'about', 'experience', 'projects', 'contact'] as const;
    const currentIndex = validSectionKeys.indexOf(activeSection as any);
    const validCurrentIndex = currentIndex >= 0 ? currentIndex : 0;
    const nextIndex = (validCurrentIndex + 1) % validSectionKeys.length;
    
    const currentThemeKey = validSectionKeys[validCurrentIndex];
    const nextThemeKey = validSectionKeys[nextIndex];
    ...
    const factor = Math.min(scrollProgress * 0.15, 1); // Even slower, smoother transitions
    ...
    return {
      ...
      glow: interpolateColor(currentTheme.glow, nextTheme.glow, factor)
    };
